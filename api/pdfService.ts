...
/**
 * PDF SERVICE (API LAYER)
 * Handles client-side PDF manipulation and extraction.
 * Relocated to api/ folder per user request to maintain architectural consistency.
 */
export class PdfService {
    ...
    if (!lib) {
      return new Promise((resolve, reject) => {
        ...
        const interval = setInterval(() => {
          ...
          if (attempts++ > 50) {
            clearInterval(interval);
            reject(new Error("PDF.js library failed to load from CDN. Please check your connection."));
          }
        }, 100);
      });
    }
  ...
  async extractText(file: File): Promise<string> {
    ...
    try {
      ...
      if (!fullText.trim()) {
        throw new Error("EMPTY_DOCUMENT_ERROR: This PDF contains no extractable text. Scanned images are not supported.");
      }
  ...
  async generateAnswerPdf(questions: QuestionItem[]): Promise<Blob> {
    // Note: jsPDF is imported via ESM from the importMap
    const doc = new jsPDF({
    ...
    const addFooter = () => {
      ...
      doc.text(`Page ${pageNumber} • AceExam Pro - Powered by Gemini`, pageWidth / 2, pageHeight - 10, { align: 'center' });
    };
    ...
    questions.forEach((item) => {
      ...
      const aLines = doc.splitTextToSize(item.answer || "No solution provided.", maxWidth);
      doc.setFont("helvetica", "normal");
      ...
      if (item.diagramDataUrl) {
         checkPage(60);
         try {
           doc.addImage(item.diagramDataUrl, 'PNG', margin, y, 100, 75);
           y += 85;
         } catch (e) {
      ...
      if (item.referenceDocUrl || item.referenceVideoUrl) {
        ...
        doc.setFont("helvetica", "bold");
        doc.setTextColor(100, 116, 139);
        doc.text("STUDY RESOURCES:", margin, y);
        y += 5;

        if (item.referenceDocUrl) {
          doc.setTextColor(37, 99, 235);
          doc.text(`Article: ${item.referenceDocUrl}`, margin, y);
          doc.link(margin, y - 3, maxWidth, 4, { url: item.referenceDocUrl });
          y += 5;
        }
        if (item.referenceVideoUrl) {
          doc.setTextColor(220, 38, 38);
          doc.text(`Video Tutorial: ${item.referenceVideoUrl}`, margin, y);
